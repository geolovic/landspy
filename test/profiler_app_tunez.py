...
class ProfilerApp:
    ...
    def _draw_chi_profile(self):
        ...
        # Draw regressions
        if len(perfil._regressions) > 0:
            for r in perfil._regressions:
                poli = r[2]
                chi1, chi2 = perfil.get_chi()[r[0]], perfil.get_chi()[r[1]]
                z1, z2 = np.polyval(poli, chi1), np.polyval(poli, chi2)
                self.ax.plot([chi1, chi2], [z1, z2], c="r", ls="--", lw=1)
#                z1, z2 = perfil.get_z()[r[0]], perfil.get_z()[r[1]]
                self.ax.plot([chi1, chi2], [z1, z2], "k+")
    ...
    def _draw_ksn_profile(self):
        ...
        if len(perfil._regressions) > 0:
            for r in perfil._regressions:
                poli = r[2]
                l1, l2 = li[r[0]] , li[r[1]]
                ksn1, ksn2 = poli[0], poli[0]
                self.ax.plot([l1, l2], [ksn1, ksn2], c="r", ls="--", lw=1)
    ...
    def change_knickpoint_type(self):
        if self.mode == "K" and len(self.profiles[self.active]._knickpoints) > 0:
            self.profiles[self.active]._knickpoints[-1][1] = (self.profiles[self.active]._knickpoints[-1][1] + 1) % len(k_type)
    ...
    def create_regression(self):
        ...
        perfil._regressions.append((p1, p2, poli, r2))
    ...
    def remove_last(self):
        ...
        """
        if self.mode == "K" and len(self.profiles[self.active]._knickpoints) > 0:
            self.profiles[self.active]._knickpoints.pop()
            self.draw()
        elif self.mode == "R" and len(self.profiles[self.active]._regressions) > 0:
            self.profiles[self.active]._regressions.pop()
            self.draw()          
    ...
    def save(self):
        ...
        np.save(self.basedir + "/perfiles.npy", self.profiles)
